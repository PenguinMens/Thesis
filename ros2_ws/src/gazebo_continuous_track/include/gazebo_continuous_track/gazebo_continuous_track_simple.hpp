...
private:
  ...
  static sdf::ElementPtr LoadPluginFormat() {
    ...
    std::string package_path = ament_index_cpp::get_package_share_directory("gazebo_continuous_track");
    std::string sdf_path = package_path + "/sdf/continuous_track_simple_plugin.sdf";

    GZ_ASSERT(sdf::initFile(sdf_path, fmt), "Cannot initialize sdf by continuous_track_simple_plugin.sdf");
